...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">simon@github: ~/contributions --graph</text>')
...
if __name__ == "__main__":
    print(f"Reading contributions from {IN_PATH} and generating heatmap {OUT_PATH}...")
    if not os.path.exists(IN_PATH):
        print(f"Error: {IN_PATH} does not exist. Please run fetch_contributions.py first!")
        sys.exit(1)
