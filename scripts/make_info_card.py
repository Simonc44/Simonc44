...
the ASCII portrait: colored key/value rows for Simon Chusseau.
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "public", "info-card.svg")
...
# content model for Simon Chusseau
ROWS = [
    ("host",),
    ("kv", "Now", "Founder @ CygnisAI"),
    ("kv", "Prev", "Founder @ Mandat (Civic Tech)"),
    ("kv", "Contact", "LinkedIn, Instagram"),
    ("kv", "Edu", "Developer & SaaS Builder"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "Next.js, React, TypeScript, Tailwind"),
    ("kv", "Backend", "Node, Serverless, Next.js API Routes"),
    ("kv", "AI / ML", "Google Gemini, LLM Orchestration"),
    ("kv", "Cloud", "Firebase, Vercel, Supabase, Git"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Founded Mandat: open-source civic tech"),
    ("bul", "Building CygnisAI: intelligent AI integrations"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">simon@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">simon</tspan><tspan fill="{MUTED}">@</tspan>'
